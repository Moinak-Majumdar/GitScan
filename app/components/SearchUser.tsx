...
const SearchUser = (props: props) => {
    ...
    return (
        <>
            <button onClick={() => dialogRef.current?.showModal()} className='ring-1 ring-slate-900/10 dark:ring-slate-700/50 shadow-sm hover:ring-slate-300 dark:hover:ring-slate-700/80 px-3 md:px-4 py-3 md:py-1 rounded-full md:rounded-lg md:h-8 flex items-center dark:bg-slate-700/30 bg-slate-200/50 cursor-pointer outline-none'>
                <IoMdSearch className='text-slate-800 dark:text-slate-300 text-xl md:text-lg' />
                <span className='text-slate-600 dark:text-slate-400 text-xs pl-3 ml-auto font-normal leading-7 '>Quick Search...</span>
                <span className='text-slate-600 dark:text-slate-300 ml-auto font-semibold text-xs pl-3'>Ctrl K</span>
            ...
            <dialog ref={dialogRef} className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-4/5 md:w-1/2 lg:w-1/3 bg-white dark:bg-slate-800 rounded-md shadow-2xl dark:shadow-black'>
                ...
                <form onSubmit={handelSubmit} className='w-full flex p-4'>
                    <input  ref={uNameRef} required style={monsterRat.style} name="github username" placeholder='Search Github username...' className="w-full bg-slate-500/10 dark:bg-white/10 rounded border border-slate-300 dark:border-slate-500 focus:border-slate-800 focus:ring-1 focus:ring-slate-200 dark:focus:ring-slate-500 text-base outline-none text-gray-700 dark:text-gray-300 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out "></input>
